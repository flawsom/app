...
type UserItem = {
  ...
  name: string;
  email: string;
  role: 'student' | 'mentor' | 'employer' | 'placement' | 'admin' | string;
  is_active: boolean;
  created_at: string;
};
...
export default function AdminDashboard() {
  ...
  const updateUserRole = useCallback(
    async (userId: string, role: string) => {
      try {
        await apiPut(`/api/users/${userId}`, { role });
        await loadData();
      } catch (e: any) {
        alert(e.message);
      }
  ...
  const toggleUserActive = useCallback(
    async (userId: string, active: boolean) => {
      try {
        await apiPut(`/api/users/${userId}`, { is_active: active });
        await loadData();
      } catch (e: any) {
        alert(e.message);
      }
    ...
    <div data-testid="admin-dashboard" className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <a
            href={`${process.env.NEXT_PUBLIC_API_URL}/api/export/applications`}
            target="_blank"
            rel="noreferrer"
            className="btn-secondary text-[10px] px-3 py-1.5 flex items-center gap-1.5 no-underline"
            data-testid="admin-export-apps"
          >
      ...
      {tab === 'overview' && analytics && (
        <div className="space-y-4">
                ...
                return (
                  <div key={role} className="text-center">
                    <div className="h-24 bg-black border border-zinc-800 relative flex items-end overflow-hidden mb-1.5 rounded-sm">
                      <div
                        className="w-full bg-[#00E5FF] transition-all duration-700"
                        style={{ height: `${pct}%` }}
                      />
                    </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="card">
              <span className="text-[10px] font-mono text-zinc-600">PLACEMENT RATE</span>
            ...
            <div className="card">
              <span className="text-[10px] font-mono text-zinc-600">CERTIFICATES</span>
          ...
          <div className="flex gap-1.5 flex-wrap">
            {['', 'student', 'mentor', 'employer', 'placement', 'admin'].map((r) => (
              <button
                ...
                className={`text-[10px] px-2.5 py-1 transition-colors rounded-sm ${
                  roleFilter === r
                ...
                <tr key={u.id} data-testid={`user-row-${u.id}`}>
                  ...
                  <td>
                    <select
                      ...
                      className="text-[10px] bg-black border border-zinc-800 px-1.5 py-0.5 text-white rounded-sm"
                  ...
                  <td className="text-zinc-600 text-xs font-mono">
      ...
      {tab === 'settings' && (
        <div className="card max-w-lg space-y-2">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold text-white">System Configuration</h3>
            <button
              onClick={seedDemo}
              disabled={seeding}
              className="btn-secondary text-[10px] px-3 py-1.5 flex items-center gap-1.5 disabled:opacity-60"
            >
          ...
          {[
            ...
            <div
              key={i}
              className="flex items-center justify-between p-2.5 bg-black border border-zinc-800 rounded-sm"
            >
              <div>
                <p className="text-xs font-medium text-white">{item.label}</p>
                <p className="text-[9px] text-zinc-600 font-mono">{item.sub}</p>
              </div>
